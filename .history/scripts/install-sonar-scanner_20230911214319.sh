...
mkdir application && cd application && git clone https://myblenet@bitbucket.org/myblenet/ui.git && cd ui && git switch pre-release && git fetch --all && git pull && git branch && cp envs/.env.sample envs/.env.local && yarn && echo -e "$(cat package.json)" | jq '.scripts' && yarn build && yarn dev
# "${opt_dir}" \
